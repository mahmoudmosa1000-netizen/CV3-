...
function init() {
  ...
  buildColorPicker(); buildFontPicker(); buildTemplatePicker(); buildSectionOrderUI();
...
// ─── TEMPLATE-AUSWAHL (Schritt 7 der Template-Architektur) ──
// Beim Wechsel bleiben alle eingegebenen Daten erhalten — es ändert
// sich nur state.template + ein re-render(). Kein Datenverlust möglich,
// weil render() für jedes Template dieselbe collectCVData()-Quelle nutzt
// und danach automatisch der Vollständigkeits-Check läuft.
...
function buildTemplatePicker() {
  const wrap=document.getElementById('template-picker'); if(!wrap) return;
  wrap.innerHTML='';
  Object.values(TEMPLATES).forEach(tpl=>{
    const el=document.createElement('div');
    el.className='template-opt'+(state.template===tpl.id?' selected':'');
    el.innerHTML=`${TEMPLATE_PREVIEWS[tpl.id]||''}<div class="template-opt-name">${t(tpl.nameKey)}</div>`;
    el.onclick=()=>{
      state.template=tpl.id;
      document.querySelectorAll('.template-opt').forEach(x=>x.classList.remove('selected'));
      el.classList.add('selected');
      render();
    };
    wrap.appendChild(el);
...
function switchTab(id,btn) {
  ...
  document.querySelectorAll('.epanel').forEach(p=>{p.classList.remove('active');p.style.display='none';});
  btn.classList.add('active');
  const panel = document.getElementById('panel-'+id);
  if(panel){ panel.style.display='block'; panel.classList.add('active'); }
  if(id==='design') setTimeout(updatePhotoDesignPreview, 50);
}
...
function addFolioLink(data = {}) {
  ...
  card.innerHTML = `
    ...
    <!-- ── ANZEIGEN ALS + POSITION (immer sichtbar) ── -->
    <div class="folio-display-grid">
      <!-- Links: Anzeigen-Optionen -->
      <div class="folio-display-col">
        <div class="folio-col-head">Anzeigen als</div>
        <label class="folio-toggle-label" style="margin-bottom:6px;">
          <input type="checkbox" id="fl-showlink-${id}" ${data.showLink!==false?'checked':''} onchange="onFolioVisibilityChange('${id}')">
          <span>🔗 Klickbarer Link</span>
        </label>
        <label class="folio-toggle-label">
          <input type="checkbox" id="fl-showqr-${id}" ${data.showQR?'checked':''} onchange="onFolioVisibilityChange('${id}')">
          <span>🔲 QR-Code</span>
        </label>
      </div>
      <!-- Rechts: Position -->
      <div class="folio-display-col">
        <div class="folio-col-head">Position im CV</div>
        <button class="folio-pos-pill ${pos==='left'?'active':''}" onclick="setFolioLinkPos('${id}','left')" id="fl-pill-left-${id}">
          ⬅ Links
          <span class="folio-pos-desc">Seitenleiste</span>
        </button>
        <button class="folio-pos-pill ${pos==='right'?'active':''}" onclick="setFolioLinkPos('${id}','right')" id="fl-pill-right-${id}">
          ➡ Rechts
          <span class="folio-pos-desc">Hauptbereich</span>
        </button>
        <button class="folio-pos-pill ${pos==='overlay'?'active':''}" onclick="setFolioLinkPos('${id}','overlay')" id="fl-pill-overlay-${id}" style="margin-top:4px;">
          📌 Frei
          <span class="folio-pos-desc">Overlay auf CV</span>
        </button>
      </div>
    </div>

    <!-- Overlay controls (nur bei pos=overlay) -->
    <div id="fl-overlay-controls-${id}" style="${pos==='overlay'?'':'display:none;'}margin-top:8px;padding:10px;background:#f5f5f5;border-radius:7px;">
      <div style="font-size:10px;color:var(--ink-faint);margin-bottom:8px;font-weight:600;">📍 Frei-Position auf dem CV (in %)</div>
...
function setFolioLinkPos(id, pos) {
  ...
  ['left','right','overlay'].forEach(p => {
    const btn = document.getElementById('fl-pill-' + p + '-' + id);
    if (btn) btn.classList.toggle('active', p === pos);
  });
  // Show/hide overlay controls (only for overlay position)
...
function onFolioVisibilityChange(id) {
  const obj = getFolioLinkObj(id);
  if (!obj) return;
  obj.url      = val('fl-url-' + id);
  obj.label    = val('fl-label-' + id);
...
function onFolioToggleQR(id) { onFolioVisibilityChange(id); }
...
function importJsonResume(d){
  ...
  if(d._cvbuilder){
    ...
    buildColorPicker();buildFontPicker();buildTemplatePicker();
  }
...
// ─── RENDER ─────────────────────────────────────
// ═══════════════════════════════════════════════
//  DATENSCHICHT — unabhängig vom Layout/Template
//  Schritt 1 der Template-Architektur: ALLE Felder
//  landen hier, egal wo sie später hingerendert werden.
// ═══════════════════════════════════════════════
// ═══════════════════════════════════════════════
//  TEMPLATE-REGISTRY — Schritt 2 der Template-Architektur
//  Jedes Template beschreibt seine Fähigkeiten (Capabilities).
//  Layouts ohne Sidebar müssen "links"-Inhalte automatisch
//  in den Hauptfluss umleiten, statt sie zu verlieren.
// ═══════════════════════════════════════════════
const TEMPLATES = {
  sidebar: {
    id: 'sidebar',
    nameKey: 'tplSidebarName',      // Übersetzungsschlüssel für den Anzeigenamen
    hasSidebar: true,               // hat eine linke Spalte
    supportsLeftRight: true,        // Nutzer darf Komps/Hobbies/Führerschein links ODER rechts platzieren
  },
  single: {
    id: 'single',
    nameKey: 'tplSingleName',
...
function getActiveTemplate(){
  return TEMPLATES[state.template] || TEMPLATES.sidebar;
}

function collectCVData(){
  const LIC=['AM','A1','A2','A','B','BE','C1','C1E','C','CE','D1','D','T','L'];
  const tpl = getActiveTemplate();
  // Capability-Fallback: hat das Template keine Sidebar / erlaubt keine
  // links/rechts-Wahl, wird die Position IMMER auf 'right' (Hauptfluss)
  // erzwungen — der Inhalt wird nie verworfen, nur nicht in eine
  // nicht-existierende Spalte geschickt.
  const rawKompsPos = (document.getElementById('f-komps-pos')||{}).value||'right';
  const rawHobbiesPos = (document.getElementById('f-hobbies-pos')||{}).value||'right';
  const rawLicensePos = (document.getElementById('f-license-pos')||{}).value||'right';
  const canPlaceLeft = tpl.hasSidebar && tpl.supportsLeftRight;
  return {
    // Design/Grundeinstellungen
    color: state.color,
    font: state.font,
    fScale: parseInt((document.getElementById('f-font-scale')||{}).value||'100')/100,
    lineH: (document.getElementById('f-line-height')||{}).value||'1.75',
    rightBg: (document.getElementById('f-right-bg')||{}).value||'#ffffff',

    // Kopf/Kontakt
    name: val('f-name')||t('cvFallbackName'),
    role: val('f-role')||(currentLang==='ar'?'المسمى الوظيفي':currentLang==='en'?'Job Title':'Berufsbezeichnung'),
    email: val('f-email'), phone: val('f-phone'), address: val('f-address'),
    birth: val('f-birth'), web: val('f-web'), linkedin: val('f-linkedin'),
    webLabel: val('f-web-label')||t('placeholderWebLabel'),
    summary: val('f-summary'), goal: val('f-goal'), komps: val('f-komps'),

    // Portfolio/Folio
    folio: val('f-folio'),
    folioLabel: val('f-folio-label')||t('cvFolio')||'Portfolio',
    ...
    photoShape: (document.getElementById('f-photo-shape')||{}).value||'circle',
    photoSize: (document.getElementById('f-photo-size')||{}).value||'120',
    borderSize: (document.getElementById('f-border-size')||{}).value||'4',
    borderColor: (document.getElementById('f-border-color')||{}).value||'rgba(255,255,255,0.9)',

    // Hobbies / Führerschein / Positionierung (links vs. rechts)
    hobbies: val('f-hobbies'),
    hobbiesSize: (document.getElementById('f-hobbies-size')||{}).value||'10',
    activeLic: LIC.filter(c=>{const el=document.getElementById('lic-'+c);return el&&el.checked;}),
    licNote: val('f-license-note'),
    kompsPos: canPlaceLeft ? rawKompsPos : 'right',
    hobbiesPos: canPlaceLeft ? rawHobbiesPos : 'right',
    licensePos: canPlaceLeft ? rawLicensePos : 'right',
    rawKompsPos, rawHobbiesPos, rawLicensePos,
    template: tpl,

    // Strukturierte Listen (Skills/Sprachen/Erfahrung/Bildung/Zusatz/Referenzen/Zertifikate/Projekte)
    skills: state.skills.map(id=>({id, name:val('sk-name-'+id), pct:val('sk-pct-'+id)})).filter(s=>s.name),
    langs: state.langs.map(id=>({id, name:val('ln-name-'+id), level:val('ln-lvl-'+id)})).filter(l=>l.name),
    exp: state.exp.map(id=>({id, title:val('exp-title-'+id), company:val('exp-company-'+id), from:val('exp-from-'+id), to:val('exp-to-'+id), desc:val('exp-desc-'+id)})).filter(e=>e.title||e.company),
    edu: state.edu.map(id=>({id, degree:val('edu-degree-'+id), school:val('edu-school-'+id), from:val('edu-from-'+id), to:val('edu-to-'+id)})).filter(e=>e.degree||e.school),
    extraquals: state.extraquals.map(id=>({id, title:val('eq-title-'+id), detail:val('eq-detail-'+id)})).filter(e=>e.title),
    refs: state.refs.map(id=>({id, name:val('ref-name-'+id), pos:val('ref-pos-'+id), company:val('ref-company-'+id), email:val('ref-email-'+id), phone:val('ref-phone-'+id), note:val('ref-note-'+id)})).filter(r=>r.name),
    certs: state.certs.map(id=>({id, title:val('cert-title-'+id), issuer:val('cert-issuer-'+id), date:val('cert-date-'+id), url:val('cert-url-'+id)})).filter(c=>c.title),
    projects: state.projects.map(id=>({id, title:val('proj-title-'+id), url:val('proj-url-'+id), from:val('proj-from-'+id), to:val('proj-to-'+id), desc:val('proj-desc-'+id)})).filter(p=>p.title),

    // QR-Overlay (Hauptwebsite)
    qrInCV: (document.getElementById('qr-in-cv')||{}).checked,
    qrSizePx: parseInt((document.getElementById('qr-size')||{}).value || '64'),
    qrX: parseFloat((document.getElementById('qr-x')||{}).value ?? '4'),
    qrY: parseFloat((document.getElementById('qr-y')||{}).value ?? '4'),
    qrCaption: val('qr-caption'),
    qrLabelPos: (document.getElementById('qr-label-pos')||{}).value || 'none',
    qrDataUrl: _qrDataUrl,

    // Seite 2
    page2Title: val('p2-title'),
    page2Free: val('p2-freetext'),
    page2Entries: state.page2Entries.map(id=>({id, title:val('p2e-title-'+id), sub:val('p2e-sub-'+id), desc:val('p2e-desc-'+id)})).filter(e=>e.title||e.desc),
...
// ═══════════════════════════════════════════════
//  VOLLSTÄNDIGKEITS-CHECK — Schritt 4 der Template-Architektur
//  Vergleicht, welche ausgefüllten Felder in collectCVData()
//  auch tatsächlich im gerenderten HTML auftauchen. Sicherheitsnetz
//  gegen "beim Template-Wechsel ist plötzlich ein Feld weg".
// ═══════════════════════════════════════════════
function buildChecklist(data){
  const items=[];
  const push=(label,v)=>{ if(v!=null && String(v).trim()) items.push({label, value:String(v).trim()}); };
  push('Name',data.name); push('Rolle',data.role); push('E-Mail',data.email); push('Telefon',data.phone);
  push('Adresse',data.address); push('Geburtsdatum',data.birth);
  if(data.linkedin) push('LinkedIn-Link vorhanden','LinkedIn');
  push('Zusammenfassung',data.summary); push('Ziel',data.goal);
  data.komps.split('\n').forEach((k,i)=>{if(k.trim())push(`Kompetenz #${i+1}`,k.trim());});
  data.hobbies.split(',').forEach((hb,i)=>{if(hb.trim())push(`Hobby #${i+1}`,hb.trim());});
  push('Führerschein-Notiz',data.licNote);
  if(data.web) push('Website-Link-Text',data.webLabel);
  if(data.folio && data.folioShowLink) push('Portfolio-Link-Text',data.folioLabel);
  data.skills.forEach(s=>push('Skill: '+s.name, s.name));
  data.langs.forEach(l=>push('Sprache: '+l.name, l.name));
  data.exp.forEach((e,i)=>{push(`Erfahrung #${i+1} Titel`,e.title); push(`Erfahrung #${i+1} Firma`,e.company); push(`Erfahrung #${i+1} Beschreibung`,e.desc);});
  data.edu.forEach((e,i)=>{push(`Bildung #${i+1} Abschluss`,e.degree); push(`Bildung #${i+1} Schule`,e.school);});
  data.extraquals.forEach((e,i)=>{push(`Zusatzqual. #${i+1} Titel`,e.title); push(`Zusatzqual. #${i+1} Detail`,e.detail);});
  data.refs.forEach((r,i)=>{push(`Referenz #${i+1} Name`,r.name); push(`Referenz #${i+1} Firma`,r.company); push(`Referenz #${i+1} E-Mail`,r.email); push(`Referenz #${i+1} Telefon`,r.phone); push(`Referenz #${i+1} Notiz`,r.note);});
  data.certs.forEach((c,i)=>{push(`Zertifikat #${i+1} Titel`,c.title); push(`Zertifikat #${i+1} Aussteller`,c.issuer);});
  data.projects.forEach((p,i)=>{push(`Projekt #${i+1} Titel`,p.title); push(`Projekt #${i+1} Beschreibung`,p.desc);});
  data.folioLinks.forEach((l,i)=>push(`Folio-Link #${i+1}`,l.label||l.url));
  push('Seite 2 – Titel',data.page2Title); push('Seite 2 – Freitext',data.page2Free);
  data.page2Entries.forEach((e,i)=>{push(`Seite 2 Eintrag #${i+1} Titel`,e.title); push(`Seite 2 Eintrag #${i+1} Beschreibung`,e.desc);});
  return items;
}

function checkRenderCompleteness(data){
  const items=buildChecklist(data);
  const containers=['cv-left','cv-right','cv-left-2','cv-right-2'].map(id=>document.getElementById(id)).filter(Boolean);
  const combinedText=containers.map(c=>c.textContent||'').join(' \n ');
  const missing=items.filter(it=>!combinedText.includes(it.value));
  if(missing.length){
    console.warn('[CV-Vollständigkeits-Check] Diese ausgefüllten Felder erscheinen NICHT im gerenderten Lebenslauf ("'+data.template.id+'"):', missing.map(m=>m.label));
  }
  const ind=document.getElementById('completeness-indicator');
  if(ind){
    if(missing.length){
      ind.style.display='inline';
      ind.textContent='⚠ '+missing.length+' Feld'+(missing.length===1?'':'er')+' fehlt';
      ind.title='Nicht im Lebenslauf sichtbar:\n'+missing.map(m=>'• '+m.label).join('\n');
    } else {
      ind.style.display='none';
      ind.textContent=''; ind.title='';
    }
  }
  return missing;
}
...
// ═══════════════════════════════════════════════
//  RENDER-DISPATCHER — wählt anhand von state.template
//  die passende Render-Funktion. Neue Templates werden
//  hier per TEMPLATE_RENDERERS registriert (Schritt 3).
// ═══════════════════════════════════════════════
function render(){
  const data=collectCVData();
  const renderFn = TEMPLATE_RENDERERS[data.template.id] || TEMPLATE_RENDERERS.sidebar;
  renderFn(data);
  checkRenderCompleteness(data);
}

function renderTemplateSidebar(data){
  const col=data.color, font=data.font;
  const colLight=lighten(col,0.55), colDark2=colLight2(col);
  const fScale=data.fScale;
  const lineH=data.lineH;
  const rightBg=data.rightBg;

  const name=data.name;
  const role=data.role;
  const email=data.email,phone=data.phone,address=data.address,birth=data.birth,web=data.web,linkedin=data.linkedin;
  const webLabel=data.webLabel, summary=data.summary, goal=data.goal, komps=data.komps;
  const folio=data.folio, folioLabel=data.folioLabel;
  const folioShowLink=data.folioShowLink;
  const folioShowQR=data.folioShowQR;
  // Collect active multi folio links
  const activeFolioLinks=data.folioLinks;

  const photoSrc=data.photoData;
  const photoShape=data.photoShape;
  ...
  const hobbiesSize=data.hobbiesSize;
  const activeLic=data.activeLic;
  const licNote=data.licNote;
  const kompsPos=data.kompsPos;
  const hobbiesPos=data.hobbiesPos;
  const licensePos=data.licensePos;
  ...
  if(activeSkills.length){
    ...
    activeSkills.forEach(sk=>{const n=sk.name,p=Math.min(100,Math.max(0,parseInt(sk.pct)||50));leftHTML+=`<div class="cv-skill-bar"><div class="cv-skill-name" style="font-size:${Math.round(11*fScale)}px;">${h(n)}</div><div class="cv-skill-track"><div class="cv-skill-fill" style="width:${p}%"></div></div></div>`;});
    leftHTML+=`</div>`;
  }
  const activeLangs=data.langs;
  if(activeLangs.length){
    leftHTML+=`<div class="cv-l-section"><div class="cv-l-section-title">${t('cvLanguages')}</div>`;
    activeLangs.forEach(lang=>{const n=lang.name,lv=lang.level;
  ...
  const renderers={
    ...
    experience:()=>{
      const entries=data.exp;
      if(!entries.length) return '';
      let s=`<div class="cv-section-head" style="color:${col};font-size:${Math.round(8.5*fScale)}px;">${t('cvExperience')}</div>`;
      entries.forEach(e=>{s+=`<div class="cv-entry" style="border-inline-start-color:${colLight};"><div class="cv-entry-head"><span class="cv-entry-title" style="font-size:${Math.round(12.5*fScale)}px;">${h(e.title)}</span><span class="cv-entry-date" style="color:${colDark2};font-size:${Math.round(10*fScale)}px;">${h(e.from)}${e.to?' – '+h(e.to):''}</span></div>${e.company?`<div class="cv-entry-sub" style="color:${colDark2};font-size:${Math.round(11*fScale)}px;">${h(e.company)}</div>`:''} ${e.desc?`<div class="cv-entry-desc" style="font-size:${Math.round(11*fScale)}px;line-height:${lineH};">${h(e.desc).replace(/\n/g,'<br>')}</div>`:''}</div>`;});
      return s;
    },
    education:()=>{
      const entries=data.edu;
      if(!entries.length) return '';
      let s=`<div class="cv-section-head" style="color:${col};font-size:${Math.round(8.5*fScale)}px;">${t('cvEducation')}</div>`;
      entries.forEach(e=>{s+=`<div class="cv-entry" style="border-inline-start-color:${colLight};"><div class="cv-entry-head"><span class="cv-entry-title" style="font-size:${Math.round(12.5*fScale)}px;">${h(e.degree)}</span><span class="cv-entry-date" style="color:${colDark2};font-size:${Math.round(10*fScale)}px;">${h(e.from)}${e.to?' – '+h(e.to):''}</span></div>${e.school?`<div class="cv-entry-sub" style="color:${colDark2};font-size:${Math.round(11*fScale)}px;">${h(e.school)}</div>`:''}</div>`;});
      return s;
    },
    komps:()=>{
    ...
    extraquals:()=>{
      const eqEntries=data.extraquals;
      ...
      if(eqEntries.length){s+=`<div class="cv-komps" style="margin-top:4px;">`;eqEntries.forEach(e=>{s+=`<div class="cv-komp" style="border-inline-start-color:${colLight};font-size:${Math.round(10.5*fScale)}px;"><span style="font-weight:700;">${h(e.title)}</span>${e.detail?`<span style="font-size:${Math.round(10*fScale)}px;color:#888;display:block;margin-top:2px;">${h(e.detail)}</span>`:''}</div>`;});s+=`</div>`;}
      return s;
    },
    referenzen:()=>{
    ...
    projekte:()=>{
      ...
      projs.forEach(p=>{const link=p.url?(p.url.startsWith('http')?p.url:'https://'+p.url):'';s+=`<div class="cv-entry" style="border-inline-start-color:${colLight};"><div class="cv-entry-head"><span class="cv-entry-title" style="font-size:${Math.round(12.5*fScale)}px;">${link?`<a href="${link}" target="_blank" style="color:${col};text-decoration:none;">`:''}${h(p.title)}${link?`</a>`:''}</span><span class="cv-entry-date" style="color:${colDark2};font-size:${Math.round(10*fScale)}px;">${h(p.from)}${p.to?' – '+h(p.to):''}</span></div>${p.url?`<div style="font-size:${Math.round(10*fScale)}px;color:${col};margin-top:2px;"><a href="${link}" target="_blank" style="color:${col};">${h(p.url)}</a></div>`:''} ${p.desc?`<div class="cv-entry-desc" style="font-size:${Math.round(11*fScale)}px;line-height:${lineH};">${h(p.desc).replace(/\n/g,'<br>')}</div>`:''}</div>`;});
      return s;
    },
    // ── FOLIO als sortierbare Sektion (rechte Spalte, pos='right') ──
    folio:()=>{
      // Right column: links with pos='right', QRs with pos='right'
      const rightLinks = activeFolioLinks.filter(l => l.showLink && l.pos === 'right');
      const rightQRs   = activeFolioLinks.filter(l => l.showQR && l.qrDataUrl && l.pos === 'right');
      if(!rightLinks.length && !rightQRs.length) return '';
      ...
      if(rightLinks.length){
        s += `<div style="display:flex;flex-direction:column;gap:4px;margin-bottom:${rightQRs.length?'10px':'0'};">`;
  ...
  const qrInCV    = data.qrInCV;
  const qrSizePx  = data.qrSizePx;
  const qrX       = data.qrX;
  const qrY       = data.qrY;
  const qrCaption = data.qrCaption;
  const qrLabelPos= data.qrLabelPos;
...
// Seite 2 ist bewusst templateunabhängig (immer im Sidebar-Stil) — sie ist ein
// Anhang/Fortsetzungsblatt, kein Haupt-Layout. So bleiben auch dort alle Felder
// sichtbar, egal welches Haupt-Template gerade aktiv ist.
function renderPage2Sheet(data, ctx){
  const {col, colLight, colDark2, font, fScale, lineH, name, role, email, phone, web, webLabel} = ctx;
  const p2Title=data.page2Title, p2Free=data.page2Free, p2Entries=data.page2Entries;
  ...
  p2Entries.forEach(e=>{right2+=`<div class="cv-entry" style="border-inline-start-color:${colLight};margin-bottom:12px;"><div class="cv-entry-head"><span class="cv-entry-title">${h(e.title)}</span></div>${e.sub?`<div class="cv-entry-sub" style="color:${colDark2};">${h(e.sub)}</div>`:''} ${e.desc?`<div class="cv-entry-desc">${h(e.desc).replace(/\n/g,'<br>')}</div>`:''}</div>`;});
...
// ═══════════════════════════════════════════════
//  TEMPLATE: EINSPALTIG / KLASSISCH
//  Kein Sidebar — Foto klein oben zentriert, alles darunter
//  in einer Spalte. Komps/Hobbies/Führerschein landen hier
//  IMMER im Hauptfluss (kompsPos/hobbiesPos/licensePos werden
...
function renderTemplateSingleColumn(data){
  ...
  const contactBits=[];
  if(email) contactBits.push(`<span>${h(email)}</span>`);
  if(phone) contactBits.push(`<span>${h(phone)}</span>`);
  if(address) contactBits.push(`<span>${h(address)}</span>`);
  if(birth) contactBits.push(`<span>${h(birth)}</span>`);
  if(web){const href=web.startsWith('http')?web:'https://'+web;contactBits.push(`<span><a href="${href}" target="_blank" style="color:${col};text-decoration:underline;font-weight:600;">${h(webLabel)}</a></span>`);}
  if(linkedin){const href=linkedin.startsWith('http')?linkedin:'https://'+linkedin;contactBits.push(`<span><a href="${href}" target="_blank" style="color:${col};text-decoration:underline;font-weight:600;">LinkedIn</a></span>`);}
  ...
  const renderEntries=(entries,titleKey,titleField,subField)=>{
    ...
    entries.forEach(e=>{
      out+=`<div class="cv-entry" style="border-inline-start-color:${colLight};"><div class="cv-entry-head"><span class="cv-entry-title" style="font-size:${Math.round(12.5*fScale)}px;">${h(e[titleField])}</span><span class="cv-entry-date" style="color:${colDark2};font-size:${Math.round(10*fScale)}px;">${h(e.from)}${e.to?' – '+h(e.to):''}</span></div>${e[subField]?`<div class="cv-entry-sub" style="color:${colDark2};font-size:${Math.round(11*fScale)}px;">${h(e[subField])}</div>`:''}${e.desc?`<div class="cv-entry-desc" style="font-size:${Math.round(11*fScale)}px;line-height:${lineH};">${h(e.desc).replace(/\n/g,'<br>')}</div>`:''}</div>`;
    });
  };
...
function applyData(d){
  ...
  buildColorPicker();buildFontPicker();buildTemplatePicker();updateCharCount();render();
}

// ─── SAVE / LOAD ─────────────────────────────────
function saveData(silent=false){
  const ind=document.getElementById('autosave-indicator');
  if(ind){ind.className='saving';ind.style.color='rgba(255,255,255,0.4)';}
  localStorage.setItem('cvbuilder_data',JSON.stringify(collectData()));
  if(ind){setTimeout(()=>{ind.className='saved';ind.style.color='rgba(255,255,255,0.5)';setTimeout(()=>{ind.className='';ind.style.color='rgba(255,255,255,0.3)';},3000);},200);}
...
function readPhoto(file){
  ...
  reader.onload=e=>{
    ...
    img.onload=()=>{
      ...
      render();updateProgress();updatePhotoDesignPreview();showToast(t('toastPhotoUploaded'));
    };
    img.src=e.target.result;
  };
  reader.readAsDataURL(file);
}
function removePhoto(){state.photoData='';document.getElementById('photo-input').value='';document.getElementById('photo-preview-img').src='';document.getElementById('photo-drop-zone').style.display='block';document.getElementById('photo-preview-section').style.display='none';render();updateProgress();updatePhotoDesignPreview();}

// ─── START ───────────────────────────────────────
init();
setTimeout(initWheelZoom, 600);
...
async function runAI() {
  const aiType = document.getElementById('ai-type')?.value || 'summary';
  const aiInput = document.getElementById('ai-input')?.value?.trim();
  const aiLang = document.getElementById('ai-lang-out')?.value || 'Deutsch';

  if (!aiInput) {
    document.getElementById('ai-error').style.display = 'block';
    document.getElementById('ai-error').textContent = '⚠ Bitte geben Sie zuerst Ihre Stichpunkte ein.';
    return;
  }
  ...
  const prompts = {
    summary: `Schreibe ein professionelles Kurzprofil für einen Lebenslauf auf ${aiLang}. Ton: ${_aiTone}. Länge: 3-4 Sätze. Nutze diese Infos: ${aiInput}. Antworte NUR mit dem Text, keine Überschriften oder Erklärungen.`,
    experience: `Schreibe eine professionelle Berufserfahrungs-Beschreibung für einen Lebenslauf auf ${aiLang}. Ton: ${_aiTone}. Nutze Aufzählungspunkte (•). Basiere dich auf: ${aiInput}. Antworte NUR mit dem Text.`,
    skills: `Liste professionelle Skills und Stärken für einen Lebenslauf auf ${aiLang} auf. Ton: ${_aiTone}. Je Zeile ein Skill, kurz und präzise. Basiere dich auf: ${aiInput}. Antworte NUR mit der Liste.`,
    cover: `Schreibe eine professionelle Anschreiben-Einleitung (2-3 Sätze) auf ${aiLang}. Ton: ${_aiTone}. Nutze diese Infos: ${aiInput}. Antworte NUR mit dem Text.`,
    improve: `Verbessere folgenden Text stilistisch für einen Lebenslauf auf ${aiLang}. Mache ihn ${_aiTone}er, prägnanter und professioneller. Behalte den Inhalt bei. Text: ${aiInput}. Antworte NUR mit dem verbesserten Text.`,
  };
  ...
  try {
    ...
    document.getElementById('ai-error').style.display = 'block';
    document.getElementById('ai-error').textContent = '❌ Fehler: ' + (err.message || 'Unbekannter Fehler. Bitte versuchen Sie es erneut.');
  } finally {
...
function aiCopyToClipboard() {
  if (!_aiLastResult) return;
  navigator.clipboard.writeText(_aiLastResult).then(() => showToast('✓ Text kopiert!')).catch(() => {
    // Fallback
    const ta = document.createElement('textarea');
    ta.value = _aiLastResult; document.body.appendChild(ta);
    ta.select(); document.execCommand('copy'); document.body.removeChild(ta);
    showToast('✓ Text kopiert!');
  });
}

function aiApplyToField() {
  if (!_aiLastResult) return;
  const el = document.getElementById('f-summary');
  if (el) { el.value = _aiLastResult; renderDebounced(); updateProgress(); updateCharCount(); showToast('✓ In Kurzprofil übernommen!'); }
}

// ─── FOTO DESIGN LIVE-VORSCHAU ────────────────────
function updatePhotoDesignPreview() {
  const wrap = document.getElementById('photo-design-avatar');
  if (!wrap) return;
  const size    = parseInt((document.getElementById('f-photo-size')||{}).value || '120');
  const border  = parseInt((document.getElementById('f-border-size')||{}).value || '4');
  const bColor  = (document.getElementById('f-border-color')||{}).value || 'rgba(255,255,255,0.9)';
  const shape   = (document.getElementById('f-photo-shape')||{}).value || 'circle';
  const radius  = shape === 'square' ? '10px' : '50%';
  // Scale down to max 100px for the mini preview
  const scale   = Math.min(1, 100 / size);
  const dispSz  = Math.round(size * scale) + 'px';
  const col     = state.color || '#3b4f3a';
  const colL    = lighten(col, 0.55);
  const name    = val('f-name') || '';
  const initials = name.split(' ').map(w=>w[0]||'').slice(0,2).join('').toUpperCase() || 'CV';

  if (state.photoData) {
    wrap.innerHTML = `<img src="${state.photoData}"
      style="width:${dispSz};height:${dispSz};border-radius:${radius};
             object-fit:cover;object-position:center top;
             border:${border}px solid ${bColor};
             box-shadow:0 4px 18px rgba(0,0,0,0.30);display:block;
             transition:all 0.2s;">`;
  } else {
    wrap.innerHTML = `<div style="width:${dispSz};height:${dispSz};border-radius:${radius};
      background:linear-gradient(145deg,${colL} 0%,${col} 100%);
      border:${border}px solid ${bColor};
      font-size:${Math.round(size*scale*0.28)}px;font-weight:700;color:#fff;
